...
cc.Class({
    ...
    onCollisionEnter: function (other, self) {
        ...
        if (cc.Intersection.rectRect(selfPreAabb, otherPreAabb)) {//在X轴方向上发生碰撞
            if (this.speed.x < 0 && (selfPreAabb.xMax > otherPreAabb.xMax)) {//在障碍物右边发生碰撞
                this.node.x = otherPreAabb.xMax - this.node.parent.x;
                this.collisionX = -1;
            }
            else if (this.speed.x > 0 && (selfPreAabb.xMin < otherPreAabb.xMin)) {//在障碍物左边发生碰撞
        ...
        if (cc.Intersection.rectRect(selfPreAabb, otherPreAabb)) {//在Y轴上发生碰撞
            if (this.speed.y < 0 && (selfPreAabb.yMax > otherPreAabb.yMax)) {//在障碍物上边发生碰撞
                this.node.y = otherPreAabb.yMax - this.node.parent.y;
                this.jumping = false;
                this.collisionY = -1;
            }
            else if (this.speed.y > 0 && (selfPreAabb.yMin < otherPreAabb.yMin)) {//在障碍物下边发生碰撞
